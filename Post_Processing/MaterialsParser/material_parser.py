...
class MaterialParser:
    def __init__(
        ...
        self.__ions = json.loads(
            open(os.path.join(self.__filename, "rsc/ions_dictionary.json")).read()
        )
        self.__anions = {
            ion["c_name"]: {
                "valency": ion["valency"],
        ...
        self.__pubchem_dictionary = json.loads(
            open(os.path.join(self.__filename, "rsc/pubchem_dict.json"), "rb").read()
        )
        self.__abbreviations = json.loads(
            open(os.path.join(self.__filename, "rsc/abbreviations.json"), "rb").read()
        )

        self.__fails_log = fails_log
        if fails_log:
            self.__pubchem_file = open(
                os.path.join(self.__filename, "pubchem_log"), "w"
            )
        ...
        if dictionary_update:
            self.__dictionary_file = open(
                os.path.join(self.__filename, "dictionary_update"), "w"
            )
    ...
    def parse_material(self, material_string_):
        ...
        if all(
            part[1:].islower()
            for part in re.sub(r"[IV\(\)]", "", material_name).split(" ")
            if part[1:] != ""
        ):
            ...
            if material_formula != "" and self.__dictionary_update:
                with open(
                    os.path.join(self.__filename, "dictionary_update"), "a"
                ) as f_dictionary:
        ...
        material_formula = (
            material_string if material_formula == "" else material_formula
        )
        ...
        for compound, amount in material_parts:
            try:
                ...
                output_structure["composition"].append(
                    dict(formula=compound, amount=amount, elements={})
                )

        if hydrate != "":
            output_structure["composition"].append(
                dict(formula="H2O", amount=hydrate, elements={"H": "2.0", "O": "1.0"})
            )
        ...
        output_structure["additives"] = [
            elem.strip(" ")
            for additive in output_structure["additives"]
            for elem in re.split(r"[\s,]", additive)
            if elem != ""
        ]
        additive = additives[0].strip(" ") if len(additives) == 1 else ""
        # print('-->', additive)
        if additive != "" and all(
            c["elements"] != {} for c in output_structure["composition"]
        ):
        ...
        try:
            if any(
                ...
                for e, s in compound["elements"].items()
            ):
                output_structure["composition"] = []
        except BaseException:
            pass
    ...
    def get_structure_by_formula(self, formula):
        ...
        if "H2O" in formula and any(c in formula for c in ["·", "•", "-", "×", "⋅"]):
            formula = (
                formula[1:-1] if formula[0] == "(" and formula[-1] == ")" else formula
            )
        ...
        for el, amt in composition.items():
            if el not in self.__list_of_elements_1 + self.__list_of_elements_2 + list(
                elements_variables.keys()
            ) + ["□"]:
        ...
        for v1, v2 in rename_variables:
            if (
                v1 in elements_variables
                and v2 in elements_variables
                and v1 + v2 in formula
            ):
        ...
        formula_structure = dict(
            elements=composition,  # {e: s for e, s in composition.items()},
    ...
    def __get_sym_dict(self, f, factor):
        ...
        r = (
            ...
            + "".join(self.__greek_letters)
            + r"\+\/]*)"
        )

        def get_code_value(code, iterator):
            code_mapping = {
                "01": (iterator.group(1), iterator.group(2)),
        ...
        for m in re.finditer(r, f):
            ...
            """
            el_bin = "{0}{1}".format(
                str(int(m.group(1)[0] in self.__list_of_elements_1 + ["M", "□"])),
                str(
                    int(
                        m.group(1)
                        in self.__list_of_elements_1
                        + self.__list_of_elements_2
                        + ["Ln", "M", "□"]
                    )
                ),
    ...
    def split_material_name(self, material_string):
        ...
        if (
            composition_e != {}
            and formula_e not in self.__list_of_elements_1 + self.__list_of_elements_2
        ):
            split = split[:-1]
            ...
            composition_b != {}
            and formula_b not in self.__list_of_elements_1 + self.__list_of_elements_2
        ):
            split = split[1:]
        ...
        name_terms = [
            p
            for p in split
            if p.lower().strip("., -;:").rstrip("s") in self.__chemicals
            or "hydrate" in p
        ]
    ...
    def reconstruct_formula(self, material_name, valency=""):
        ...
        _, anion_prefix_num, anion = (
            ("", 0, anion)
            if anion.lower() in self.__anions
            else self.__get_prefix(anion)
        )
        ...
        if hydrate != "":
            ...
            hydrate_prefix = (
                "" if hydrate_prefix_num in [0, 1] else str(hydrate_prefix_num)
            )
    ...
    def __build_formula(self, cation, anion, cation_prefix_num=0, anion_prefix_num=0):
        ...
        if (
            anion_prefix_num + cation_prefix_num == 0
            or anion_prefix_num * cation_prefix_num != 0
        ):
        ...
        return "{0}{1}{2}{3}".format(
            cation_name_el,
            self.__cast_stoichiometry(cation_stoich),
            anion_name_el,
    ...
    def __split_name(self, material_name_, init_fraction="1"):
        ...
        re_str = (
            "(?<=[0-9\)])[-⋅·∙\∗](?=[\(0-9])|(?<=[A-Z])[-⋅·∙\∗](?=[\(0-9])|(?<=[A-Z\)])[-⋅·∙\∗](?=[A-Z])|(?<=["
            r"0-9\)])[-⋅·∙\∗](?=[A-Z])"
        )
        re_str = re_str + "".join(
            [
                "|(?<=" + e + r")[-⋅·∙\∗](?=[\(0-9A-Z])"
                for e in self.__list_of_elements_1 + self.__list_of_elements_2
            ]
        )
        ...
        if any(
            m.strip("0987654321")
            in self.__list_of_elements_1 + self.__list_of_elements_2
    ...
    def get_additives(self, material_name):
        ...
        for r in ["coated", "activated", "modified", "stabilized", "doped"]:
            parts = [
                w
                for w in re.split(r"(.*)[-\s]{1}" + r + " (.*)", new_material_name)
                if w != ""
            ]
            if len(parts) > 1:
        ...
        for part_ in new_material_name.split(":"):
            ...
            if all(
                e.strip("zyx,+0987654321. ")
                in self.__list_of_elements_1 + ["R"] + ["&&"]
                for e in re.split(r"[\s,/]", part)
                if e != ""
            ):
    ...
    def __is_abbreviation(self, word):
        if (
            all(c.isupper() for c in re.sub(r"[0-9x\-\(\)\.]", "", word))
            and len(re.findall("[A-NP-Z]", word)) > 1
        ):
            return True
    ...
    def build_abbreviations_dict(self, materials_list, paragraph):
        ...
        abbreviations_dict = {
            ...
            if self.__is_abbreviation(t.replace(" ", "")) and t != ""
        }
        ...
        for abbr, name in abbreviations_dict.items():
            sents = (
                " ".join([s + " " for s in paragraph if abbr in s])
                .strip(" ")
                .split(abbr)
            )
            i = 0
        ...
        for abbr in abbreviations_dict.keys():
            parts = re.split("-", abbr)
            if (
                all(p in abbreviations_dict for p in parts)
                and abbreviations_dict[abbr] == ""
                and len(parts) > 1
            ):
                name = "".join(
                    "(" + abbreviations_dict[p] + ")" + "-" for p in parts
                ).rstrip("-")
    ...
    def __get_values(self, string, mode):
        ...
        if mode == "range":
            ...
            try:
                ...
                min_value = (
                    round(float(smp.simplify(min_value)), 4)
                    if min_value != ""
                    else None
                )
        ...
        if mode == "values":
            ...
            try:
                values = [
                    round(float(smp.simplify(c.rstrip("., "))), 4)
                    for c in values
                    if c.rstrip("., ") not in ["", "and"]
                ]
    ...
    def get_stoichiometric_values(self, var, sentence):
        ...
        regs = [
            (var + "\s*=\s*([-]{0,1}[0-9\.\,/and\s]+)[\s\)\]\,]", "values"),
    ...
    def is_materials_list(self, material_string):
        if (
            any(a + "s" in material_string.lower() for a in self.__anions.keys())
            or "metal" in material_string
        ) and any(w in material_string for w in ["and ", ",", " of "]):
            return True
    ...
    def reconstruct_list_of_materials(self, material_string):
        ...
        cation = [
            (i, p)
            for i, p in enumerate(parts)
            if p.lower() in self.__cations.keys()
    ...
    def cleanup_name(self, material_name):
        ...
        if (
            len(material_name) == 2
            and material_name not in self.__list_of_elements_2
            and material_name.rstrip("234") not in self.__list_of_elements_1
            and any(c not in self.__list_of_elements_1 for c in material_name)
        ):
            return ""
    ...
    def __simplify(self, value):
        ...
        for i, m in enumerate(
            ...
            new_value = (
                new_value[0 : m.start(1) + i] + "*" + new_value[m.start(1) + i :]
            )
    ...
    def __substitute_additive(self, additive, material_formula, material_composition):
        ...
        if (
            coeff == []
            or element not in self.__list_of_elements_1 + self.__list_of_elements_2
        ):
            return new_material_formula, material_composition

        for compound in material_composition:
            expr = "".join(
                ["(" + v + ")+" for e, v in compound["elements"].items()]
            ).rstrip("+")

            coeff = (
                coeff[0]
                if not re.match("^[0]+[1-9]", coeff[0])
                else "0." + coeff[0][1:]
            )
            expr = expr + "+(" + coeff + ")"
            ...
            if self.__is_int(self.__simplify(expr)):
                ...
                new_material_composition.append(
                    dict(
                        formula=new_name,
    ...
    def __is_abbreviation_like(self, structure):
        ...
        elements_vars = [
            el
            for el in structure["elements_vars"].keys()
            if len(el) == 1 and el.isupper()
        ]
        ...
        if re.findall("[A-Z]{3,}", structure["material_formula"]) != [] and all(
            w not in structure["material_formula"] for w in ["CH", "COO", "OH"]
        ):
            return True
