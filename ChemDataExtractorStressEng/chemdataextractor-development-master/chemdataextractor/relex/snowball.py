...
class Snowball(BaseSentenceParser):
    ...
    def __init__(
        ...
        suffix_length=1,
        learning_rate=0.5,
        max_candidate_combinations=30,  # changed from 400 to 30
    ...
    def train(self, corpus, start=0, end=None):
        ...
        if isinstance(corpus, str):
            ...
            for i, file_name in enumerate(corpus_list[start:end]):
                print("{}/{}:".format(i + start + 1, len(corpus_list)), " ", file_name)
                f = os.path.join(corpus, file_name)
                self.train_from_file(f)
        else:
            assert isinstance(corpus, list)
            for s in corpus[start:end]:
        ...
        if len(candidate_relationships) > 0:
            ...
            if self.play_sound:
                sound_file = pkg_resources.resource_filename(
                    "chemdataextractor", "eval/sound.mp3"
                )
    ...
    def candidates(self, tokens):
        ...
        for field in self.model.fields:
            if self.model.fields[field].required and all(
                field not in key for key in entities_dict.keys()
            ):
