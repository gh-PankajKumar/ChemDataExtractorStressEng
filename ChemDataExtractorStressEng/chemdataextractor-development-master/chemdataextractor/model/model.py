...
from .units.stress import StressModel
...
from ..parse.grain_size import GrainSizeParser, MultiGrainSizeParser
from ..parse.yield_strength import YieldStrengthParser, MultiYieldStrengthParser
from ..parse.uts import TensileStrengthParser, MultiTensileStrengthParser
...
# Models added for CDE Yield Strength & Grain Size
class GrainSize(LengthModel):
    """Model for grain size text parsing"""

    specifier_expression = (
        Optional(R("[Aa]verage")) + R("^[Gg]rain$") + R("^[Ss]ize")
    ).add_action(join)
    ...
    parsers = [GrainSizeParser(), MultiGrainSizeParser()]
...
class YieldStrength(StressModel):
    ...
    specifier = StringType(parse_expression=specifier_expression, required=True)
    compound = ModelType(Compound, contextual=True, required=True)
    parsers = [YieldStrengthParser(), MultiYieldStrengthParser()]


class TableGrainSize(LengthModel):
    """ Model for grain size table parsing"""
