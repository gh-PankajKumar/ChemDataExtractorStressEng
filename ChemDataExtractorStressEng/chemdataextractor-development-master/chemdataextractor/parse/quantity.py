...
def value_element(
    ...
    spaced_range = (
        number + Optional(units).hide() + (R("^[\-–−~∼˜]$") + number | number)
    ...
    value_single = (Optional(R("^[~∼˜\<\>]$")) + Optional(R("^[\-–−]$")) + number)(
        "raw_value"
    ).add_action(merge)
    value = (
        Optional(lbrct).hide()
            ...
            for index, string in enumerate(split):
                if string in found_units.keys():
                    ...
                    if found_units[string] == prev_unit:
                        ...
                        prev_unit
                        and re.match(_magnitude_indicators, split[index - 1])
