...
class BaseAutoParser(BaseParser):
    ...
    def interpret(self, result, start, end):
        x = etree.tostring(result)
        y = xml.dom.minidom.parseString(x)
        print(y.toprettyxml(indent="\t"))
        if result is None:
            return
        requirements = True
        property_entities = {}
...
class AutoSentenceParser(BaseAutoParser, BaseSentenceParser):
    def __init__(
        self, lenient=False, chem_name=(cem | chemical_label | lenient_chemical_label)
    ):
...
class AutoTableParser(BaseAutoParser, BaseTableParser):
    ...
    def root(self):
        ...
        if hasattr(self.model, "dimensions") and not self.model.dimensions:
            ...
            value_phrase = (value_element_plain() | no_value_element) + Optional(
                unit_element
            )
            entities.append(specifier)
            entities.append(value_phrase)
